...
function transformToLis(obj){
  ...
  for (let i = 0; i < keys.length; i++) {
    result.push(`<li>${keys[i]}: ${obj[keys[i]]}</li>`);
  }
  return result;
}
...
const divisibleByFiveTwoToThePower = (input) => {
  if (Array.isArray(input)) {
    return input
      .filter((val) => typeof val === 'number' && val % 5 === 0)
...
let findMaleAndFemale = (data) => {
  if (Array.isArray(data)) {
    return data
      .filter((val) => val.gender === 'male' || val.gender === 'female')
      .map((val) => val.name)
      .join(' and ');
  }
  return '';
...
let findShortest = (data) => {
  if (Array.isArray(data)) {
    return data
      .filter((val) => typeof val.height === 'number')
      .reduce((acc, val) => {
        return acc.height < val.height ? acc : val;
      }).name;
  }
  return '';
};
